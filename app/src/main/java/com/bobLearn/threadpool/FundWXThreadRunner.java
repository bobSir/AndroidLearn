...
public class FundWXThreadRunner implements Callback, IThreadRunner {
    ...
    private static FundWXThreadRunner sTaskHolder;
    //线程池
    ...
    public static synchronized FundWXThreadRunner getInstance() {
        if (sTaskHolder == null)
            sTaskHolder = new FundWXThreadRunner();
        return sTaskHolder;
    }
     ...
     * @param group    任务分组
     */
    public synchronized void runTask(Callable<?> callable, Callback callback, String group, FundWXThreadPriority priority) {
        if (callable == null)
            return;
    ...
    public boolean handleMessage(Message msg) {
        synchronized (this) {
            ...
            if (task != null) {
                ...
                if (!task.isCancelled) {
                    ...
                    if (callback != null) {
                        ...
                        try {
                            ...
                            Logger.w(LOG_TAG + "--->>future get result failed:" + e.getMessage());
                ...
                Logger.i(LOG_TAG + "--->>An removed task has finished" + (null != task ? (" is cancelled? " + task.isCancelled) : ""));
            //根据优先级获取需要执行的任务
            ...
            while (interator.hasNext()) {
                ...
                if (TextUtils.isEmpty(toRun.mGroupName)) {
                    //没有group的被挂起执行啦
                    Logger.i(LOG_TAG + "--->>an task dose not belong to any group has not been excute Immediately");
                    interator.remove();
                    ...
                    if (groupinfo == null) {
                        groupinfo = new GroupInfo();
                        Logger.i("first task belongs to a group has not been excute Immediately");
    ...
    private static class Task {
        ...
        private FundWXThreadPriority mPriority;
