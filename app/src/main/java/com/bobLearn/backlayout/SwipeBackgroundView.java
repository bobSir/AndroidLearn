    ...
    public void bind(Activity activity, Activity swipeActivity, boolean restoreForSubWindow) {
        ...
        if (orientation != getResources().getConfiguration().orientation) {
            ...
            if (requestedOrientation == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE ||
                ...
                mDoRotate = true;
            } else if (swipeActivity instanceof BaseBackActivity) {
                swipeActivity.getWindow().getDecorView().setBackgroundColor(0);
                ((BaseBackActivity) swipeActivity).convertToTranslucentCauseOrientationChanged();
                invalidate();
                return;
            }
        }
        ...
        void draw(Canvas canvas) {
            ...
            if (view != null) {
                if (isMain || lp == null) {
                    ...
                    canvas.drawColor(ColorHelper.setColorAlpha(Color.BLACK, lp.dimAmount));
